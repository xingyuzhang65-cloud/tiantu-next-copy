...
export default function InstructionListPage({ addToast }: { addToast?: (text: string, type?: 'success' | 'info' | 'warning') => void }) {
  ...
  const exportRows = () => {
    ...
    const csv = [['运单号', '指令单号', '指令名称', '创建时间', '指令处理状态', '处理结果', '客户简称', '仓库代码', '下单类型', '指令单类型', '拦截原因'].join(','), ...picked.map((row) => [row.waybill, row.instructionNo, row.names.join(' / '), row.created, row.status, getProcessingResult(row), row.customer, row.warehouse, row.orderType, row.instructionType, row.instructionType === '拦截' ? (row.interceptReason || '') : ''].map((value) => `"${value.replace(/"/g, '""')}"`).join(','))].join('\n');
