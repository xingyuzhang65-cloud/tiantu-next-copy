...
type Row = {
  ...
  shipment: string; reference: string; status: Status; photo?: string; customer: string; warehouse: string; zip: string;
...
export default function InstructionListPage({ addToast }: { addToast?: (text: string, type?: 'success' | 'info' | 'warning') => void }) {
  ...
  const exportRows = () => {
    ...
    const csv = [['运单号', '指令单号', '指令名称', '创建时间', '指令处理状态', '客户简称', '仓库代码', '下单类型', '指令单类型'].join(','), ...picked.map((row) => [row.waybill, row.instructionNo, row.names.join(' / '), row.created, row.status, row.customer, row.warehouse, row.orderType, row.instructionType].map((value) => `"${value.replace(/"/g, '""')}"`).join(','))].join('\n');
